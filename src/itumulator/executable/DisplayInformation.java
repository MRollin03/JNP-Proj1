...
/**
 * An abstraction used to determine the visual appearance of objects within the
 * simulation.
 * Images have precedence over colors, however, if a class implements
 * {@link DynamicDisplayInformationProvider}, this preceeds images.
 */
public class DisplayInformation {
     ...
     * @param random_direction Whether or not the image can be placed in a random
     *                         direction.
     */
    public DisplayInformation(Color color, String imageKey, boolean random_direction) {
        if (color == null || imageKey == null)
    ...
    /**
     * Display using an image (and color when image is not applicable). Will always
     * use the same image orientation.
     * 
     * @param color    to fall back upon.
     * @param imageKey to the image
     */
